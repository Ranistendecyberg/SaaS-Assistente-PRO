-- Cadastro atômico da empresa, proprietário e computador principal da versão 2.0.
-- O usuário já deve existir no Supabase Auth e ter confirmado o e-mail.
...
create index if not exists onboarding_attempts_user_time_idx
  on public.onboarding_attempts(user_id, attempted_at desc);

alter table public.onboarding_attempts enable row level security;
alter table public.onboarding_attempts force row level security;
revoke all on table public.onboarding_attempts from public, anon, authenticated;
grant select, insert, update, delete on table public.onboarding_attempts to service_role;
...
comment on table public.onboarding_attempts is
  'Controle técnico de tentativas do onboarding; não armazena o identificador bruto do computador.';
